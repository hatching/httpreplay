...
from httpreplay.protohandlers import http_handler, https_handler, smtp_handler
...
def httpreplay(pcapfile, tlsmaster):
    ...
    reader.set_tcp_handler(TCPPacketStreamer(reader, handlers))

    for s, ts, protocol, sent, recv in reader.process():
        print(s, ts, protocol, getattr(sent, "uri", None))
